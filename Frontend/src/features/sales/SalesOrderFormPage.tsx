...
export function SalesOrderFormPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: (values: FormValues) => (isNew ? createSalesOrder(values) : updateSalesOrder(orderId as number, values)),
    onSuccess: (o) => {
      invalidate()
      if (isNew) navigate(`/sales/orders/${o.id}`, { replace: true })
    },
    onError: (err) => setServerError(getApiErrorMessage(err)),
  })

  const confirmMutation = useMutation({
    mutationFn: () => confirmSalesOrder(orderId as number),
    onSuccess: invalidate,
    onError: (err) => setServerError(getApiErrorMessage(err)),
  })

  const cancelMutation = useMutation({
    mutationFn: () => cancelSalesOrder(orderId as number),
    onSuccess: invalidate,
    onError: (err) => setServerError(getApiErrorMessage(err)),
  })

  const createInvoiceMutation = useMutation({
    mutationFn: () => createInvoiceFromOrder(orderId as number),
  ...
  if (!isNew && status === 'CONFIRMED') {
    actions.push({
      label: createInvoiceMutation.isPending ? 'Creating…' : 'Create Invoice',
  ...
  return (
    ...
    >
      <div className="grid grid-cols-3 gap-x-6 gap-y-4 mb-6">
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-[var(--color-ink)]">SO Date</label>
